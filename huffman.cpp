...
void huffman::traverse(Node* r, std::string str) {
...
int huffman::binToDec(std::string inStr) {
    int res = 0;
    for (std::string::size_type i = 0; i < inStr.length(); ++i) {  // Use traditional for loop
        char c = inStr[i];
        res = res * 2 + c - '0';
    }
    return res;
...
std::string huffman::decToBin(int inNum) {
    std::string temp = "", res = "";
    while (inNum > 0) {
        temp += (inNum % 2 + '0');
        inNum /= 2;
...
void huffman::buildTree(char a_code, std::string& path) {
    Node* curr = root;
    for (std::string::size_type i = 0; i < path.length(); ++i) {  // Use traditional for loop
        if (path[i] == '0') {
            if (curr->left == NULL) {
                curr->left = new Node();
            }
...
void huffman::saveDecodedFile() {
    ...
    Node *curr = root;
    std::string path;
    for (std::vector<unsigned char>::size_type i = 0; i < text.size() - 1; ++i) {  // Use traditional for loop
        ...
        for (std::string::size_type j = 0; j < path.size(); ++j) {  // Use traditional for loop
            if (path[j] == '0') {
                curr = curr->left;
            }
            else {
